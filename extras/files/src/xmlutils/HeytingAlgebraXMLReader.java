...
public class HeytingAlgebraXMLReader extends XMLReader<HeytingAlgebra> implements XMLNodeConverter<HeytingAlgebra>{
    private static HeytingAlgebraXMLReader READER = null;

    public static HeytingAlgebraXMLReader getInstance() {
        if (READER == null) READER = new HeytingAlgebraXMLReader();
        return READER;
    }

    private HeytingAlgebraXMLReader() {
        setXMLSchema(XSDPATH + "heyting.xsd");
